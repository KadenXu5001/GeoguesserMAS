...
function VisionScreen({ round, state, setState, onBack }) {
  useEffect(() => {
    ...
    analyzeRound(round.roundId).then((payload) => {
      setState((current) => ({ ...current, analysis: payload.analysis, informedEvidence: payload.informedEvidence || [], predictedCountry: payload.predictedCountry, alternativeCountries: payload.alternativeCountries || [], analysisLoading: false }));
    }).catch((error) => {
...
function App() {
  ...
  const [roundState, setRoundState] = useState({ selectedCountry: null, result: null, viewState: { yaw: 0, pitch: 0, zoom: 35 }, analysis: null, informedEvidence: [], predictedCountry: null, alternativeCountries: [], selectedEvidenceId: null, showAgentSees: false, showAgentInforms: true, analysisLoading: false, analysisError: "", loadingNext: false });
  ...
  const resetRoundState = () => setRoundState({ selectedCountry: null, result: null, viewState: { yaw: 0, pitch: 0, zoom: 35 }, analysis: null, informedEvidence: [], predictedCountry: null, alternativeCountries: [], selectedEvidenceId: null, showAgentSees: false, showAgentInforms: true, analysisLoading: false, analysisError: "", loadingNext: false });
