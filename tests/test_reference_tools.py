...
def test_specialist_tools_query_only_allowed_categories() -> None:
    assert lookup_universal_clues.func("driving_side", runtime(), "Thailand")[0]["version"] == "reference-v1"
    assert lookup_rural_clues.func("rural_architecture", runtime(), "Thailand")[0]["category"] == "rural_architecture"


def test_specialist_tools_batch_a_category_when_country_is_omitted() -> None:
    result = lookup_rural_clues.func("vegetation_biomes", runtime())
...
def test_specialist_tools_reject_repeated_category_lookup() -> None:
    context = runtime()
    lookup_rural_clues.func("vegetation_biomes", context)
    result = lookup_rural_clues.func("vegetation_biomes", context)
...
def test_unsupported_specialist_category_returns_recoverable_tool_feedback() -> None:
    result = lookup_rural_clues.func("infrastructure", runtime())
